...
class Curl
{
     ...
     * @return mixed
     */
    public static function commonPost(string $url, string $cacert_url = null, $para = null)
    {
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, $cacert_url ? true : false);//SSL证书认证
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, $cacert_url ? 2 : 0);//严格认证
        ...
        // fixme: 设置下面这个post头可能会产生错误(用于文件上传？！)
     ...
     * @return mixed
     */
    public static function get(string $url, string $cacert_url = null, $getHeader = false)
    {
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_HEADER, 0); // 过滤HTTP头
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);// 显示输出结果
