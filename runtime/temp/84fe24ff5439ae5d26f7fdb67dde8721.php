                ...
                <div class="tpl-block">
                    <div class="am-g">
                        <div class="am-u-sm-12 am-u-md-6">
                            <div class="am-btn-toolbar">
                                <div class="am-btn-group am-btn-group-xs">
                                    <a href="/article/edit" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 新增</a>
                        ...
                        <div class="am-u-sm-12 am-u-md-3">
                            <div class="am-form-group" id="articleType">
                                <select data-am-selected="{btnSize: 'sm'}">
                                    <?php if(is_array($parent_category_list) || $parent_category_list instanceof \think\Collection || $parent_category_list instanceof \think\Paginator): $i = 0; $__LIST__ = $parent_category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?>
                                    <option value="<?php echo $category['id']; ?>" <?php if($type == $category['id']): ?>selected="selected"<?php endif; ?>><?php echo $category['category_name']; ?></option>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                  <!-- <option value="2" <?php if($type == 2): ?>selected="selected"<?php endif; ?>>新闻</option>
                                  <option value="3" <?php if($type == 3): ?>selected="selected"<?php endif; ?>>菜谱</option>
                                  <option value="4" <?php if($type == 4): ?>selected="selected"<?php endif; ?>>技术</option> -->
                                </select>
                            </div>
                        </div>
                                ...
                                <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            ...
                                            <!-- <th class="table-type">副標題</th>
                                            <th class="table-author am-hide-sm-only">slogan</th> -->
                                            <th class="table-date am-hide-sm-only">封面圖</th>
                                            <!-- <th class="table-date am-hide-sm-only">标签</th> -->
                                            <th class="table-date am-hide-sm-only">首页推荐</th>
                                            <th class="table-date am-hide-sm-only">热门文章</th>
                                            <th class="table-date am-hide-sm-only">狀態</th>
                                            <th class="table-set">操作</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                        <?php if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $i = 0; $__LIST__ = $article_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?>
                                        <tr>
                                            <td><input type="checkbox"></td>
                                            <td><?php echo $article['id']; ?></td>
                                            <td class="am-hide-sm-only"><?php echo $article['author_id']; ?></td>
                                            <td><a href="#"><?php echo $article['title']; ?></a></td>
                                            <!-- <td><?php echo $article['sub_title']; ?></td>
                                            <td><?php echo $article['slogan']; ?></td> -->
                                            <td class="am-hide-sm-only"><i class="am-icon-<?php echo $article['cover_image']; ?>"></i></td>
                                            <!-- <td class="am-hide-sm-only"><?php echo $article['category']; ?></td> -->
                                            <td>
                                                <div class="tpl-switch">
                                                    <input type="checkbox" id="index" name="index" class="ios-switch bigswitch tpl-switch-btn" <?php if($article['index'] == 1): ?>checked<?php endif; ?> />
                                                    <div class="tpl-switch-btn-view" onclick="changePram(<?php echo $article['id']; ?>, 'index')">
                                                        <div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                <div class="tpl-switch">
                                                    <input type="checkbox" id="hot" name="hot" class="ios-switch bigswitch tpl-switch-btn" <?php if($article['hot'] == 1): ?>checked<?php endif; ?> />
                                                    <div class="tpl-switch-btn-view" onclick="changePram(<?php echo $article['id']; ?>, 'hot')">
                                                        <div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                <div class="tpl-switch">
                                                    <input type="checkbox" id="status" name="status" class="ios-switch bigswitch tpl-switch-btn" <?php if($article['status'] == 1): ?>checked<?php endif; ?> />
                                                    <div class="tpl-switch-btn-view" onclick="changePram(<?php echo $article['id']; ?>, 'status')">
                                                        <div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
                                                        ...
                                                        <a class="am-btn am-btn-default am-btn-xs am-text-secondary a-btn" onclick="doEdit(<?php echo $article['id']; ?>)">
                                                            <!-- <a href="/article/edit?id=<?php echo $article['id']; ?>"> -->
                                                            <span class="am-icon-pencil-square-o"></span> 编辑
                                                            <!-- </a> -->
                                                        </a>

                                                        <!-- <a class="am-btn am-btn-default am-btn-xs am-hide-sm-only a-btn" onclick="doCopy(<?php echo $article['id']; ?>)">
                                                            <span class="am-icon-copy"></span> 复制
                                                        </a> -->

                                                        <a class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only a-btn" onclick="doDel(<?php echo $article['id']; ?>)">
...
$("#articleType select").on('change', function(){
    var type = $(this).val();
    var page = $(".am-active a").text();
    window.location.href = '/article?type='+type;
})

function doEdit(id){
    window.location.href = API_URL+'/article/edit?id='+id;
}

function doDel(id){
    window.location.href = API_URL+'/article/del?id='+id;
}
...
function changePram(id, parm){
    $.ajax({
        url:API_URL+'/article/changePram',
        type:'POST',
        data:{"id":id, "key":parm},
        dataType:'json',
